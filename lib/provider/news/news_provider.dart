...
class NewsProvider {
  ...
  final ValueNotifier<List<Article>> _favoriteNews = ValueNotifier(<Article>[]);
  ...
  ValueNotifier<List<Article>> get favoriteNews => _favoriteNews;
  ...
  final isFavorite = _favoriteNews.value.contains(article);
  final updatedList = List<Article>.from(_favoriteNews.value);
  ...
  _favoriteNews.value = updatedList;
}

  void deleteFavoriteNews(Article article) {
  _favoriteNews.value = _favoriteNews.value.where((a) => a != article).toList();
}
